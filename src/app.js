...
showAnswer = (qs) => {
  const space = '                                                     ';
  const answer = typeof(qs[questionNum].answer) === 'string' ? qs[questionNum].answer : qs[questionNum].answer.join('\n');
  console.log(colors.green('\033[2A' + `${space}\n${space}`+ answer))
}

showQuestion = (question) => {
  console.log(colors.bold.yellow('________________________________________________________\n\n'))
...
const helpMsg = {
  guide: 'To update the selected question set, type one of the following: A , B , AB ',
...
rl.on('line', (input) => {
  ...
  if(!running){
    if(input === 'a' || input === 'b' || input === 'ab') {
      ...
      console.log(colors.grey('type [ start ] to begin, or type [ help ] for more information'));
    }
  } 
  // if questions have started
    ...
    if (input === 'skip') {
      handleMoveOn(qs);
        ...
        if(input === ''){
          showingAnswer = true;
          console.log(colors.grey('are you sure?\n<<< press ENTER to move on >>>'));
        }
    }
    else {
      if(input === 'y'){
